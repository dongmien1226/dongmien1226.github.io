document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("./src/js/archive.json")
        .then((res) => res.json())
        .then((archive) => {
            archiveContainer.innerHTML = ""; // Clear existing content

            Object.keys(archive).forEach(tag => {
                const tagSection = document.createElement("div");
                tagSection.classList.add("tag-section");

                const postsHTML = archive[tag]
                    .map(post => `<li><a href="/blog.html?post=${post.filename}" class="post-title">${post.title}</a></li>`)
                    .join('');

                tagSection.innerHTML = `
                    <h2 class="tag-title">${tag}</h2>
                    <ul class="tag-posts">${postsHTML}</ul>
                `;

                archiveContainer.appendChild(tagSection);
            });
        })
        .catch((err) => {
            console.error("Error fetching archive:", err);
            archiveContainer.innerHTML = "<p>Failed to load archive.</p>";
        });
});
